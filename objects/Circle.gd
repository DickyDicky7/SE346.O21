...
func init(_position, level = 1):
	...
	if randf() < move_chance:
		...
		move_speed = max(  2.5 - ceil(    level / 05) *  0.25,                  0.75                                            );
	...
	var  img_size = $Sprite.texture.get_size().x / 2.0;
...
func draw_circle_arc_poly(
	...
	color ):
	var     nb_points = 32;
	var points_arc    = PoolVector2Array(); points_arc.push_back(center);
	var colors =                            PoolColorArray(
	   [color]);
	
	for i in range(nb_points + 1):
		var angle_point = angle_from  + i * (
			angle_to    - angle_from) / nb_points - PI/2;
		points_arc.push_back(center + Vector2(
			cos(angle_point),
